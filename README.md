...
Создайте файл `~/.kaiten/config` с настройками доступа:
```bash
mkdir -p ~/.kaiten
cat > ~/.kaiten/config << EOF
KAITEN_API_URL=https://ваш-домен.kaiten.ru/api/latest
...
**Примечание**: 
- Глобальные настройки (`~/.kaiten/config`) имеют приоритет
- Если не указать `cwd` в настройках, команды выполняются в текущей директории проекта
- Это используется для интеграции с Claude через `mcpServers`
...
# Git интеграция
kaiten git-branch <card_id>             # Создать ветку для задачи
kaiten git-checkout <card_id>           # Переключиться на ветку задачи
kaiten git-commit <card_id> [msg]       # Закоммитить изменения (msg по умолчанию: "Work in progress")
kaiten git-status                     # Показать статус git
kaiten git-push <card_id>              # Запушить ветку
...
Для работы с git используй команды через `kaiten`:
```bash
kaiten git-branch <card_id>          # Создать ветку feature/<id>-<title>
kaiten git-checkout <card_id>         # Переключиться на ветку
kaiten git-commit <card_id> <msg>   # Закоммитить с привязкой к задаче
kaiten git-status                     # Показать статус git
kaiten git-push <card_id>              # Запушить ветку
```
...
kaiten git-branch <card_id>          # feature/<id>-<title>

# 3. Внести изменения и закоммитить
# ...работа над кодом...
kaiten git-commit <card_id> "Начал работу"    # [<card_id>] Начал работу

# 4. Проверить статус
kaiten git-status

# 5. Запушить
kaiten git-push <card_id>              # git push origin feature/<id>-<title>
```
